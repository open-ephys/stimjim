...
typedef struct {
    ...
    // pt_cmd holds a command-initiated pulsetrain so pt_active has somewhere
    // stable to point when not using pt_trigger.
    pulsetrain_t pt_cmd;
    ...
    // pin constants
    bool channel;
    uint8_t channel_io_pin;
    uint8_t led_pin;
    uint8_t nldac_pin;
} stimulus_context_t;

static pio_spi_t pio_spi[2] __attribute__((section(".scratch_x")));
static stimulus_context_t sc[2] __attribute__((section(".scratch_x")));
...
// inline version of pico SDK function to minimize potential pulse train jitter
...
__always_inline static inline void process_offsets_queue(stimulus_context_t *sc) {
    ...
    while (queue_try_remove(&q_offsets_tx, &offsets_calibration)) {
        offsets_t offsets[2];
...
void __time_critical_func(main_core1)(void) {
    ...
    static const pio_spi_t pio_spi_defaults[2] = {
