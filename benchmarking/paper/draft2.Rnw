...
#matplot(sapply(res[1:60], function(x) x$pulseMat[,1]), col=c(1,2), type='l', lty=1)
...
#matplot(dendRites::offset(sapply(res[61:120], function(x) x$pulseMat[,1])), col=c(1,2), type='l', lty=1)
...
plot(paramMat[61:90,5], sapply(res[seq(121,180,by=2)], function(x) x$sh.median*1e3), 
  log='', main='amplitude vs target', xlab='Target amplitude (mV)', ylab='measured amplitude (mV)', cex=0.75)
abline(a=0,b=1.0)
points(paramMat[61:90,5], sapply(res[seq(122,180,by=2)], function(x) x$sh.median*1e3),
  col='red', cex=0.75)
