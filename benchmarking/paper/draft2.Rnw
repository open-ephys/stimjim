...
\hline
 ...
 Compliance voltage             & $\pm$15~V (voltage) & $\pm$10~V  & $\pm$120~V                        & $\pm$45~V                & $\pm$100~V     \\
                                & $\pm$13.7~V (current)    &       &                                   &                          &                \\
 Fastest pulse                  & $\approx$30~$\mu$s      & 100~$\mu$s       & 20~$\mu$s                         & 60~$\mu$s               & 10~$\mu$s      \\
...
gplots::plotCI(x,y[2,], li=y[1,], ui=y[3,], yaxt='n', lty=1, gap=0, sfrac=0,
...
par(mar=c(4,3,2,1))
plot(paramMat[31:59,7]/1e3, sapply(res[seq(61,118,by=2)], function(x) x$PW.summary[3])*1e3/2,  yaxt='n',
...
gplots::plotCI(x,y[1,], li=y[1,], ui=y[3,], yaxt='n', range=0, lty=1, gap=0, sfrac=0,
...
<<sliceExpt, eval=FALSE>>=
if (!require(readABF)){
  install.packages("devtools")
  devtools::install_github("Zabolekar/abfReader")
}

sliceDat = readABF::readABF("./slice2/27_06_19_0016.abf")$data[[1]][,1]
...
aps = sapply(i, function(j) {
  x = sliceDat[j+50:500]
  length(getLocalMaxima(x,threshold=0))
})
