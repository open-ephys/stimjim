...
getPulseParams = function(f, thresh=25, fs, vpau){
  ...
  apply(dat, 2, function(x) {  
    ...
    sh.median = mean(x[ishe])*vpau
    sh.sd = sd(x[ishe])*vpau
    sl.median = mean(x[isle])*vpau
...
y = t(sapply(res[seq(121,178,by=2)], function(x) c(x$sh.median,x$sl.median))*1e3) * 0.458/0.4593
